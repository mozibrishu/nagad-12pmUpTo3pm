...
function dateCalculation() {
    ...
    if (checkingHourStart <= hours && checkingHourEnd >= hours) {
        hideSlide1();
    } else {
        displaySlide1();
...
function checkRemainingTime(hours, minutes, seconds) {
    ...
    calcRemainingHours = () => {
        if (hours < checkingHourStart) {
            return remainingMinutes == 0 && minutes == 0 ? checkingHourStart - hours : (checkingHourStart - 1) - hours;
        } else {
            return remainingMinutes == 0 && minutes == 0 ? checkingHourStart + (24 - hours) : (checkingHourStart - 1) + (24 - hours);
        }
    }
